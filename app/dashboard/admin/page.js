...
import { Shield, Users, Calculator, MessageSquare, IndianRupee } from 'lucide-react'
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Calculations" value={stats.totalCalculations} icon={Calculator} />
        <StatCard label="AI sessions" value={stats.totalAISessions} icon={MessageSquare} />
        <StatCard label="MRR" value={`₹${planRevenue.toLocaleString('en-IN')}`} icon={IndianRupee} />
      </div>
            ...
            <tbody className="divide-y divide-slate-800">
              {filtered.map((u) => (
                <tr key={u.userId} className="hover:bg-slate-800/30">
                  ...
                  <td className="px-5 py-3 text-red-300">{u.role}</td>
                  <td className="px-5 py-3 text-orange-300">{u.plan}</td>
