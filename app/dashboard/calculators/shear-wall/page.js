...
function computeShearWall(input, override = {}) {
  ...
  const boundaryLength = boundaryRequired
    ? Math.max(0.15 * L, 0.3)
    : 0
  ...
  const boundaryAstProvided =
    boundaryBarsEach * barArea(m.boundaryBarDia)

  const verticalBarsPerLayer = Math.ceil((Lmm - 2 * m.cover) / verticalSpacingData.spacing) + 1
  const horizontalBarsPerLayer = Math.ceil((Hmm - 2 * m.cover) / horizontalSpacingData.spacing) + 1

  const verticalSteelKgPerWall =
    verticalBarsPerLayer *
    faces *
    H *
    barWeight(m.verticalBarDia)

  const horizontalSteelKgPerWall =
    horizontalBarsPerLayer *
    faces *
    L *
    barWeight(m.horizontalBarDia)

  const boundarySteelKgPerWall =
    boundaryRequired
      ? boundaryBarsEach *
        2 *
        H *
        barWeight(m.boundaryBarDia)
      : 0
...
function findOptimizedWall(input) {
  ...
  for (let t = 150; t <= 500; t += 25) {
    const trial = computeShearWall(input, {
      thickness: t,
    })
...
export default function ShearWallDesignPage() {
  ...
  const results = useMemo(() => computeShearWall(inputs), [inputs])
  const optimized = useMemo(() => findOptimizedWall(inputs), [inputs])
  ...
  const downloadPdf = async () => {
    try {
      ...
      const lines = [
        `Wall Length: ${round(inputs.wallLength)} ${unit.length}`,
        `Storey Height: ${round(inputs.wallHeight)} ${unit.length}`,
        `Wall Thickness: ${round(inputs.thickness)} ${unit.thickness}`,
        `Axial Load Pu: ${round(inputs.axialLoad)} ${unit.load}`,
        `Shear Force Vu: ${round(inputs.shearForce)} ${unit.shear}`,
        `Moment Mu: ${round(inputs.bendingMoment)} ${unit.moment}`,
        ...
        `Vertical Steel: ${inputs.verticalBarDia} mm @ ${results.verticalSpacing} mm c/c`,
        `Horizontal Steel: ${inputs.horizontalBarDia} mm @ ${results.horizontalSpacing} mm c/c`,
      ...
      <section className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-slate-300">
                ...
                vertical steel, horizontal steel, boundary element, quantity,
                cost aur PDF report calculate kare.
              </p>
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            {optimized ? (
              ...
              <div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-5 py-4 text-sm font-bold text-red-200">
                No safe design found. Increase wall length, reduce load, or use
                stronger section.
              </div>
            )}
          </div>
          ...
          {optimized && (
            <div className="mt-5 grid gap-4 md:grid-cols-4">
              ...
              <QuantityCard
                ...
                subtitle={`Limit ${round(optimized.results.preliminaryShearLimit)} N/mm²`}
              />

              <QuantityCard
                title="Boundary Element"
                value={optimized.results.boundaryRequired ? 'Required' : 'Not Required'}
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Field
                  ...
                  labelHi="Main vertical reinforcement"
                  name="verticalBarDia"
                  suffix="mm"
                  step="1"
                />
                ...
                <Field
                  ...
                  labelHi="Horizontal reinforcement"
                ...
                <Field
                  ...
                  labelHi="Boundary element vertical bars"
                  name="boundaryBarDia"
                  suffix="mm"
                  step="1"
                />
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-slate-800 bg-slate-950 p-6">
              ...
              <div className="rounded-2xl border border-slate-800 bg-[#071027] p-4">
                <svg viewBox="0 0 560 360" className="h-auto w-full">
                  <rect x="0" y="0" width="560" height="360" fill="#071027" />
                  ...
                  <line x1="225" y1="315" x2="335" y2="315" stroke="#93c5fd" />
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <ResultCard
                  ...
                  subtitle={`Limit ${round(results.preliminaryShearLimit)} N/mm²`}
                ...
                <ResultCard
                  ...
                  subtitle={`Limit approx ${round(0.25 * results.m.fck)} N/mm²`}
                ...
                <ResultCard
                  ...
                  subtitle={`Limit approx ${round(0.45 * results.m.fck)} N/mm²`}
                ...
                <ResultCard
                  title="Boundary Element"
                  value={results.boundaryRequired ? 'Required' : 'Not Required'}
                  subtitle={
                    results.boundaryRequired
                      ? `Zone length ${round(results.boundaryLength)} m each end`
                      : 'Normal end zone ok'
                  }
                />
              </div>
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <div className="space-y-3 text-sm text-slate-300">
                ...
                <p>✓ Vertical bars dono faces par proper spacing me place karo.</p>
                <p>✓ Horizontal bars continuous rakho aur lap location stagger karo.</p>
                <p>✓ Boundary element required ho toh extra confinement provide karo.</p>
                <p>✓ Openings, door cut-outs aur coupling beams separately design karo.</p>
                <p>✓ Foundation connection me dowel bars aur development length verify karo.</p>
                <p>✓ Concrete cover, chair support aur bar alignment site par check karo.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <tbody>
                <tr className="border-b border-slate-800">
                  ...
                  <td className="px-4 py-4 text-orange-300">
                    {inputs.verticalBarDia} mm @ {results.verticalSpacing} mm c/c
                  </td>
                  <td className="px-4 py-4 text-slate-300">
                ...
                <tr className="border-b border-slate-800">
                  ...
                  <td className="px-4 py-4 text-orange-300">
                    {inputs.horizontalBarDia} mm @ {results.horizontalSpacing} mm c/c
                  </td>
                  <td className="px-4 py-4 text-slate-300">
                ...
                <tr className="border-b border-slate-800">
                  ...
                  <td className="px-4 py-4 text-orange-300">
                    {results.boundaryRequired
                      ? `${results.boundaryBarsEach} bars of ${inputs.boundaryBarDia} mm each end`
                      : '—'}
                  </td>
                  <td className="px-4 py-4 text-slate-300">
          ...
          <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-950 p-4 text-sm text-slate-300">
            ...
            200 mm ya usse zyada ho toh generally reinforcement dono faces par
            dena practical hota hai. Boundary required ho toh end zone par
        ...
        <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <QuantityCard
              ...
              subtitle={`${round(results.concreteVolumePerWall)} m³ per wall per storey`}
            />
        ...
        <div className="mt-6 rounded-3xl border border-orange-500/20 bg-orange-500/10 p-6 text-sm leading-7 text-orange-100">
          ...
          design, site checking, quantity aur reinforcement planning ke liye hai.
          Final design me load combinations, ductile detailing, boundary element,
          coupling beam, openings, foundation connection aur applicable code
          requirements structural engineer se verify karna zaroori hai.
        </div>
      </section>
    </main>
  )
