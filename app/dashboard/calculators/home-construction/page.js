 ...
 const downloadPDF = () => {
  ...
  const addFooterToAllPages = () => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        "Generated by CivilCalc Pro - civilcalcpro.in",
        margin,
        pageHeight - 10
      );
  ...
  summaryCard(
    ...
    24,
    "Construction Cost",
    pdfMoney(result.constructionCost)
  );
  ...
  summaryCard(
    ...
    24,
    "Hidden Cost",
    pdfMoney(result.additionalHiddenCost)
  );
  ...
  summaryCard(
    ...
    24,
    "Area",
    `${result.constructionArea.toFixed(0)} sq ft`
  );
  ...
  sectionTitle("House Configuration and Optional Features", doc.lastAutoTable.finalY + 14);

  autoTable(doc, {
    startY: doc.lastAutoTable.finalY + 21,
    head: [["Item", "Quantity", "Estimated Cost"]],
    body: [
      ["Bedrooms", form.bedrooms || "0", pdfMoney(Number(form.bedrooms || 0) * 50000)],
      ["Kitchens", form.kitchens || "0", pdfMoney(Number(form.kitchens || 0) * 80000)],
      ["Bathrooms", form.bathrooms || "0", pdfMoney(Number(form.bathrooms || 0) * 60000)],
      ["Halls", form.halls || "0", pdfMoney(Number(form.halls || 0) * 40000)],
      ["Car Parking", form.carParking || "0", pdfMoney(Number(form.carParking || 0) * 100000)],
      ["Balcony", form.balcony || "0", pdfMoney(Number(form.balcony || 0) * 50000)],
      ["Terrace", form.terrace || "0", pdfMoney(Number(form.terrace || 0) * 75000)],
      ["Basement", form.basement || "0", pdfMoney(Number(form.basement || 0) * 400000)],
      ["Lift", form.lift || "0", pdfMoney(Number(form.lift || 0) * 1000000)],
    ],
    theme: "grid",
    styles: {
      fontSize: 9,
  ...
  // PAGE 5 — HIDDEN COSTS, EMI, TIMELINE
  doc.addPage();
  addHeader("Hidden Cost, EMI and Timeline");
  ...
  sectionTitle("Loan EMI Summary", doc.lastAutoTable.finalY + 14);
  ...
  autoTable(doc, {
    ...
    body: result.roomRecommendation.rooms.map((room) => [
      safeText(room.name),
      safeText(room.size),
    ]),
    theme: "grid",
  ...
  const notes = [
    ...
    "Material quantities include wastage percentages entered by the user.",
    "Rates used in this report are editable and should be updated according to local market rates before final budgeting.",
  ];
