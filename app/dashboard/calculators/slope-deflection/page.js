...
function calculateMemberSfdBmd(member, endMomentRow) {
  ...
  for (let i = 0; i <= divisions; i++) {
    ...
    const shear = RA - loadBeforeSection(member, x)
    const moment = MLeft + RA * x - loadMomentBeforeSection(member, x)
            ...
            {spanData.map(span => (
              <tr key={span.member} className="border-b border-slate-800">
                ...
                <td className="p-3 text-sky-300">
                  R{span.leftJoint} = {round(span.RA)} kN
                </td>

                <td className="p-3 text-sky-300">
                  R{span.rightJoint} = {round(span.RB)} kN
                </td>

                <td className="p-3 text-slate-300">
                  {round(span.totalLoad)} kN
                </td>

                <td className="p-3 text-slate-300">
                  {round(span.maxShear)} kN
                </td>

                <td className="p-3 text-slate-300">
                  {round(span.minShear)} kN
                </td>

                <td className="p-3 text-orange-300">
                  {round(span.maxMoment)} kNm
                </td>

                <td className="p-3 text-orange-300">
                  {round(span.minMoment)} kNm
                </td>
              </tr>
            ))}
          </tbody>
